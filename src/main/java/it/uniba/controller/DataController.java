...
public class DataController
{
    ...
    {
        view.printChannelMembers(mod.getUsers(), mod.getChannels(),
                _nameChannel);
    }

    // Uhm, manca qualcosa (?)
    public void printMentions(String channel, String nome, Boolean from,
            Boolean weight)
    {
        if (nome.equals(""))
        {
    ...
    public void printMention(final String _inChannel)
    {
        ...
        {// -m
            mod.getMentionGraph().parseMessages(mod.getMessages(),
                    mod.getUsers(), "");
            view.printMention(mod.getMentionGraph().edgesOutDegree(null));
        } else
        { // validazione canale -m in _inChannel
            if (mod.getChannels().containsKey(_inChannel))
            {
                mod.getMentionGraph().parseMessages(mod.getMessages(),
                        mod.getUsers(), _inChannel);
                view.printMention(mod.getMentionGraph().edgesOutDegree(null));
            } else
            {
    ...
    // #38
    void printMentionsFromToUser(final String _user, final String _inChannel,
            final boolean _from)
    {
        if (_user != null || _user.equals(""))
        {
            String idUser = getUserFromId(_user);
            if (mod.getUsers().containsKey(idUser)) // l'utente esiste nel
                                                    // workspace
            {
                if ((_inChannel == null || _inChannel.equals(""))
                        || mod.getChannels().containsKey(_inChannel))
                {
                    mod.getMentionGraph().parseMessages(mod.getMessages(),
                            mod.getUsers(), _inChannel);
                    ...
                    if (_from)
                    {
                        edgesneeded = mod.getMentionGraph()
                                .edgesOutDegree(mod.getUsers().get(idUser));
                    } else
                    {
                        edgesneeded = mod.getMentionGraph()
                                .edgesInDegree(mod.getUsers().get(idUser));
                    }
    ...
    public void printMentionsToUser(final String _user, final String _inChannel)
    {
        ...
        /*
         * String idUser = getUserFromId(fileParser, _user); if
         * (_inChannel.equals("")) { if
         * (fileParser.getUsers().containsKey(idUser)) // l'utente esiste nel
         * workspace {
         * fileParser.getMentionGraph().parseMessages(fileParser.getMessages(),
         * fileParser.getUsers(), "");
         * 
         * } else { System.out.println("The user specified doesn't exist."); } }
         * else { if (fileParser.getUsers().containsKey(idUser) &&
         * fileParser.getChannels().containsKey(_inChannel)) {
         * fileParser.getMentionGraph().parseMessages(fileParser.getMessages(),
         * fileParser.getUsers(), _inChannel); // parse // dei // mention // sul
         * // grafo //
         * fileParser.getMentionGraph().printEdgesInDegree(fileParser.getUsers()
         * .get( idUser)); } else { if
         * (!fileParser.getUsers().containsKey(idUser))
